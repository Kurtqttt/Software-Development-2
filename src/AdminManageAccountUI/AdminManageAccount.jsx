...
const AdminManageAccount = () => {
  ...
  return (
    <div className="account-container w-96 rounded-lg bg-gradient-to-br from-gray-400 via-gray-600 to-gray-900 max-w-800 min-h-300 flex flex-col items-center justify-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-md p-20 font-roboto">
      ...
      <div className={`form-container ${error ? 'disabled-input' : ''} flex flex-col items-center`}>
        ...
        <input
          ...
          disabled={isFormDisabled}
        />
        <button className="save-button cursor-pointer text-white rounded-lg py-2 px-8 mt-4 transition-colors duration-300 ease-in-out bg-gray-500 hover:bg-gray-800" onClick={handleSave} disabled={error !== '' || isFormDisabled}>
          Save
        </button>
      </div>
      {error && (
